...
class LinearBlendSkinning(nn.Module):
    ...
    def init(self, target, device=torch.device('cpu')):
        ...
        self.vertices_origin[:, :-1] = target.verts_origin.clone().float().to(device)
    ...
    def forward(self, x, rotation='euler'):
        ...
        _, trans = self.fk(x, self.target.parent_all.to(self.device), self.target.offset_all.to(self.device), x.shape[0], return_transform=True, rotation=rotation)
